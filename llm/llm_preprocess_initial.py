...
처음부터 LLM을 사용하여 measure와 timeframe을 파싱합니다.
(규칙 기반 파싱 전에 또는 대신 사용)
"""
        ...
        for outcome in outcomes:
            ...
            if outcome_id in result_map:
                ...
                results.append({
                    ...
                    'llm_parsed_time_points': time_points_json,
...
def update_llm_results(conn, results: List[Dict]):
    ...
    update_sql = """
        ...
        SET 
            ...
            llm_parsed_time_points = %(llm_parsed_time_points)s,
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            query = """
                ...
                WHERE measure_code IS NULL
                  AND time_value_main IS NULL
                  AND time_unit_main IS NULL
                  AND llm_parsed_measure_code IS NULL
