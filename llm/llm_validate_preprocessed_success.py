"""
LLM 전처리 성공 항목 검증 스크립트

outcome_llm_preprocessed 테이블에서 llm_status = 'SUCCESS'인 항목들을
LLM으로 검증하여 문서화합니다.
"""
...
def validate_batch_single_run(outcomes: List[Dict]) -> Dict[int, Dict]:
    """
    배치 단위로 outcome들을 LLM으로 검증 (1회 실행)
    
    Args:
        outcomes: 검증할 outcome 리스트
    
    Returns:
        {outcome_id: {status, confidence, notes}} 형태의 딕셔너리
    """
    if not outcomes:
        return {}
    ...
    if llm_config._all_keys_exhausted:
        ...
        return {
            ...
            for outcome in outcomes if outcome.get('id')
        }
    
    # 배치 프롬프트 생성
    items = []
    outcome_id_map = {}  # outcome_id -> outcome 매핑
    
    for outcome in outcomes:
        oid = outcome.get('id')
        mr = outcome.get('measure_raw', '')
        dr = outcome.get('description_raw', '')  # description_raw 추가
        tr = outcome.get('time_frame_raw', '')
        mc = outcome.get('llm_measure_code', '')
        tv = outcome.get('llm_time_value', '')
        tu = outcome.get('llm_time_unit', '')
        tp = format_time_points(outcome.get('llm_time_points'))
        
        parts = [f"{oid}"]
        if mr: parts.append(f"M:{mr}")
        if dr: parts.append(f"D:{dr}")  # description_raw 추가
    ...
    if llm_config._all_keys_exhausted:
        ...
        return {
            ...
            for oid in outcome_id_map.keys()
        }
    ...
    if isinstance(result, list):
        ...
        for oid in outcome_id_map.keys():
            if oid not in results_map:
                results_map[oid] = {
                    'status': 'UNCERTAIN',
                    'confidence': None,
                    'notes': '[PARSE_ERROR] LLM 응답에 outcome_id가 없음.'
                }
    else:
        # 단일 응답인 경우 (하위 호환성)
        if outcomes:
...
def validate_with_multi_run_for_outcome(
    outcome: Dict,
    validation_results_by_run: Dict[int, Dict],
    existing_results: List[Dict]
) -> Dict:
    """
    단일 outcome에 대해 다중 검증 결과를 처리하는 함수
    
    Args:
        outcome: 검증할 outcome 데이터
        validation_results_by_run: {run_number: {outcome_id: result}} 형태의 딕셔너리
...
def validate_batch_outcomes(outcomes: List[Dict], num_validations: int = 3, conn=None) -> tuple:
    """
    배치 단위로 outcome들을 다중 검증 (전처리와 동일한 방식)
    
    Args:
        outcomes: 검증할 outcome 리스트
        num_validations: 각 outcome당 검증 횟수 (기본값: 3)
        conn: 데이터베이스 연결 (재검증 시 기존 이력과 합치기 위해 필요)
    
    Returns:
        (results: List[Dict], validation_results_by_run: Dict[int, Dict])
        - results: 각 outcome별 최종 검증 결과
        - validation_results_by_run: {run_number: {outcome_id: result}} 형태의 검증 이력
    ...
    # 기존 검증 이력 조회 (재검증 시)
    existing_results_by_outcome = {}
    if conn:
        for outcome in outcomes:
            outcome_id = outcome.get('id')
            if outcome_id:
                existing_results = get_existing_validation_history(conn, outcome_id)
                if existing_results:
                    existing_results_by_outcome[outcome_id] = existing_results
    
    # N회 검증 수행 (배치 단위로)
    validation_results_by_run = {}  # {run_number: {outcome_id: result}}
    ...
    for run_num in range(1, num_validations + 1):
        ...
        # 배치 단위로 1회 검증
        run_results = validate_batch_single_run(outcomes)
    ...
    for outcome in outcomes:
        outcome_id = outcome.get('id')
        existing_results = existing_results_by_outcome.get(outcome_id, [])
        
        result = validate_with_multi_run_for_outcome(
            outcome,
...
def get_existing_validation_history(conn, outcome_id: int) -> List[Dict]:
    """기존 검증 이력을 조회"""
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        cur.execute("""
            ...
            FROM outcome_llm_validation_history
            WHERE outcome_id = %s
...
def save_validation_history_batch(conn, validation_results_by_run: Dict[int, Dict]):
    ...
    Args:
        conn: 데이터베이스 연결
        validation_results_by_run: {run_number: {outcome_id: result}} 형태
    """
    ...
    for run_results in validation_results_by_run.values():
        all_outcome_ids.update(run_results.keys())
    
    if not all_outcome_ids:
        return
    
    # 각 outcome_id별로 기존 이력의 최대 validation_run 조회 (배치로)
    outcome_max_runs = {}
    with conn.cursor() as cur:
        placeholders = ','.join(['%s'] * len(all_outcome_ids))
        cur.execute(f"""
            SELECT 
                outcome_id,
            ...
            GROUP BY outcome_id
        """, list(all_outcome_ids))
        
        for row in cur.fetchall():
            outcome_max_runs[row[0]] = row[1]
    
    # outcome_id별로 다음 run 번호 설정
    outcome_next_runs = {}
    for outcome_id in all_outcome_ids:
        outcome_next_runs[outcome_id] = outcome_max_runs.get(outcome_id, 0) + 1
    
    # 모든 검증 이력 데이터 수집
    history_data = []
    for run_num in sorted(validation_results_by_run.keys()):
        run_results = validation_results_by_run[run_num]
        for outcome_id, result in run_results.items():
            history_data.append({
                'outcome_id': outcome_id,
                'validation_run': outcome_next_runs[outcome_id],
                'validation_status': result.get('status'),
                'validation_confidence': result.get('confidence'),
                'validation_notes': result.get('notes', '')
            })
            # 다음 run 번호 증가
            outcome_next_runs[outcome_id] += 1
    ...
    # 배치로 저장
    history_sql = """
        INSERT INTO outcome_llm_validation_history 
        (outcome_id, validation_run, validation_status, validation_confidence, validation_notes)
        VALUES (%(outcome_id)s, %(validation_run)s, %(validation_status)s, %(validation_confidence)s, %(validation_notes)s)
    """
...
def update_validation_results(conn, results: List[Dict], validation_results_by_run: Dict[int, Dict] = None):
    ...
    Args:
        conn: 데이터베이스 연결
        results: 검증 결과 리스트
        validation_results_by_run: {run_number: {outcome_id: result}} 형태 (배치 검증 결과)
    """
    if not results:
        return
    ...
    if validation_results_by_run:
        ...
        for result in results:
            ...
            if outcome_id and validation_results:
                # 개별 저장 (하위 호환성)
                outcome_max_runs = {}
                with conn.cursor() as cur:
                    cur.execute("""
...
def generate_validation_report(conn, output_dir=None):
    ...
    report_path = f'{output_dir}/llm_preprocessed_validation_{timestamp}.md'
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        if stats and stats['total_success'] > 0:
            ...
            f.write(f'- **MEASURE_FAILED**: {stats["measure_failed"]:,}개 ({stats["measure_failed"]/total*100:.2f}%)\n')
            f.write(f'- **TIMEFRAME_FAILED**: {stats["timeframe_failed"]:,}개 ({stats["timeframe_failed"]/total*100:.2f}%)\n')
        ...
        if study_stats:
            ...
            if total_studies > 0:
                ...
                f.write(f'- **모든 outcome이 VERIFIED인 Study**: {complete_studies:,}개 ({complete_studies/total_studies*100:.2f}%)\n')
        f.write('\n')
        ...
        f.write('## 5. 검증 방법\n\n')
        f.write('1. `outcome_llm_preprocessed` 테이블에서 `llm_status = \'SUCCESS\'`인 항목들을 조회\n')
...
def main():
    ...
    print("=" * 80)
    print("[START] LLM 전처리 성공 항목 검증 시작")
    print("=" * 80)
    ...
    # 명령줄 인자 파싱
    # 사용법: python llm_validate_preprocessed_success.py [limit] [num_validations] [batch_size] [start_batch]
    limit = None
    ...
    try:
        ...
        for batch_start in range(0, total_count, actual_batch_size):
            ...
            batch_results, validation_results_by_run = validate_batch_outcomes(batch_outcomes, num_validations, conn)
        ...
        print(f"  MEASURE_FAILED: {measure_failed_count:,}개 ({measure_failed_count/total_count*100:.1f}%)")
        print(f"  TIMEFRAME_FAILED: {timeframe_failed_count:,}개 ({timeframe_failed_count/total_count*100:.1f}%)")
