"""
Study별 Outcome 분포 시각화 스크립트

Study별로 outcome 성공/실패 분포를 시각화합니다.
...
def plot_success_count_histogram(study_stats, output_dir='visualization'):
    ...
    ax.set_title('Study별 성공한 Outcome 개수 분포', fontsize=14, fontweight='bold', pad=20, fontproperties=KOREAN_FONT_PROP)
    ...
    plt.savefig(f'{output_dir}/study_outcome_success_count_histogram.png', dpi=300, bbox_inches='tight')
    print(f"[OK] 그래프 저장: {output_dir}/study_outcome_success_count_histogram.png")
    plt.close()
...
def plot_success_rate_distribution(study_stats, output_dir='visualization'):
    ...
    ax.set_ylabel('Study 수', fontsize=12, fontweight='bold', fontproperties=KOREAN_FONT_PROP)
    ax.set_title('Study별 Outcome 성공률 구간 분포 (10% 단위)', fontsize=14, fontweight='bold', pad=20, fontproperties=KOREAN_FONT_PROP)
    ax.grid(axis='y', alpha=0.3)
    ...
    plt.tight_layout()
    os.makedirs(output_dir, exist_ok=True)
    plt.savefig(f'{output_dir}/study_outcome_success_rate_distribution.png', dpi=300, bbox_inches='tight')
    print(f"[OK] 그래프 저장: {output_dir}/study_outcome_success_rate_distribution.png")
    plt.close()
...
def plot_success_vs_total_scatter(study_stats, output_dir='visualization'):
    ...
    ax.set_title('Study별 Outcome 성공 개수 vs 전체 개수', fontsize=14, fontweight='bold', pad=20, fontproperties=KOREAN_FONT_PROP)
    ax.grid(alpha=0.3)
    ...
    plt.tight_layout()
    os.makedirs(output_dir, exist_ok=True)
    plt.savefig(f'{output_dir}/study_outcome_success_vs_total_scatter.png', dpi=300, bbox_inches='tight')
    print(f"[OK] 그래프 저장: {output_dir}/study_outcome_success_vs_total_scatter.png")
    plt.close()
...
def main():
    """메인 함수"""
    print("=" * 80)
    print("[START] Study별 Outcome 분포 시각화 시작")
    print("=" * 80)
    ...
    try:
        ...
        print("=" * 80)
        print("\n생성된 파일:")
        print("  - visualization/study_outcome_success_count_histogram.png")
        print("  - visualization/study_outcome_success_rate_distribution.png")
        print("  - visualization/study_outcome_success_vs_total_scatter.png")
