...
`llm_validate_preprocessed_success.py`는 LLM으로 전처리된 성공 항목(SUCCESS)을 검증하는 스크립트입니다.
...
- 배치 크기: 100개
- 시작 배치: 1번
- 처리 대상: `llm_validation_status IS NULL`인 모든 SUCCESS 항목
...
```bash
python llm/llm_validate_preprocessed_success.py [limit] [num_validations] [batch_size] [start_batch]
```
...
```bash
# 각 항목을 5회 검증
python llm/llm_validate_preprocessed_success.py 1000 5
```
...
```bash
# 배치 크기를 50개로 설정
python llm/llm_validate_preprocessed_success.py 1000 3 50
```
...
python llm/llm_validate_preprocessed_success.py 1000 3 100 10
```
...
```sql
outcome_llm_validation_history
├── id (PK)
├── outcome_id (FK → outcome_llm_preprocessed.id)
├── validation_run (검증 실행 횟수: 1, 2, 3, ...)
├── validation_status (VERIFIED, UNCERTAIN, MEASURE_FAILED, ...)
...
---------------|-------------------|----------------------|-------------------
1              | VERIFIED          | 0.95                 | 2025-12-24 10:00:00
2              | VERIFIED          | 0.92                 | 2025-12-24 10:00:05
3              | UNCERTAIN         | 0.60                 | 2025-12-24 10:00:10
4              | VERIFIED          | 0.90                 | 2025-12-24 11:00:00
5              | VERIFIED          | 0.88                 | 2025-12-24 11:00:05
6              | VERIFIED          | 0.93                 | 2025-12-24 11:00:10
```

### 메인 테이블 컬럼

`outcome_llm_preprocessed` 테이블의 검증 관련 컬럼:
...
기본적으로 `llm_validation_status IS NULL`인 항목만 검증합니다. 재검증을 위해서는:
...
**주의**: 검증 이력(`outcome_llm_validation_history`)은 삭제되지 않으며, 재검증 시 기존 이력과 합쳐서 처리됩니다.
...
```bash
python llm/llm_validate_preprocessed_success.py 10 3
```
...
```bash
python llm/llm_validate_preprocessed_success.py 1000 5 50
```
...
- 필요시 수동으로 이력 삭제:
  ...
  WHERE outcome_id = 123 AND validation_run > 10;
  ```
...
SELECT
    ...
    h.validation_notes,
    h.created_at
FROM outcome_llm_validation_history h
WHERE h.outcome_id = 123
...
```
reports/llm_preprocessed_validation_YYYYMMDD_HHMMSS.md
```
...
모든 검증 실행 결과가 `outcome_llm_validation_history` 테이블에 저장됩니다.
...
```sql
-- 특정 outcome의 모든 이력 삭제
DELETE FROM outcome_llm_validation_history
WHERE outcome_id = 123;

-- 특정 outcome의 일부 이력만 삭제 (예: 10회차 이후)
DELETE FROM outcome_llm_validation_history
WHERE outcome_id = 123 AND validation_run > 10;
```
...
- 검증 스크립트: `llm/llm_validate_preprocessed_success.py`
- 스키마 SQL: `sql/add_multi_validation_schema.sql`
